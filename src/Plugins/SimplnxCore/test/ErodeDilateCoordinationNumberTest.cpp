...
CoordinationLiteralFixture CreateCoordinationLiteralFixture(const usize dimX, const usize dimY, const usize dimZ, const std::vector<int32>& inputFeatureIds = {1, 0, 0, 1})
{
  ...
  auto featureIdsStore = DataStoreUtilities::CreateDataStore<int32>(fixture.dataStructure, fixture.featureIdsPath, cellTupleShape, {1}, IDataAction::Mode::Execute);
  ...
  auto transferredValuesStore = DataStoreUtilities::CreateDataStore<int32>(fixture.dataStructure, fixture.transferredValuesPath, cellTupleShape, {1}, IDataAction::Mode::Execute);
  ...
  auto ignoredValuesStore = DataStoreUtilities::CreateDataStore<int32>(fixture.dataStructure, fixture.ignoredValuesPath, cellTupleShape, {1}, IDataAction::Mode::Execute);
...
{
  SIMPLNX_RESULT_REQUIRE_VALID(RunCoordinationLiteralFixture(fixture, threshold));
}
...
DataPath AddCoordinationSibling(CoordinationLiteralFixture& fixture, const std::string& name, const std::array<T, N>& inputValues, usize componentCount)
{
  ...
  auto store = DataStoreUtilities::CreateDataStore<T>(fixture.dataStructure, path, cellDataRef.getShape(), {componentCount}, IDataAction::Mode::Execute);
  REQUIRE(store->getStoreType() == IDataStore::StoreType::InMemory);
  REQUIRE(store->getSize() == N);
  SIMPLNX_RESULT_REQUIRE_VALID(store->copyFromBuffer(0, nonstd::span<const T>(inputValues.data(), inputValues.size())));
...
void BuildTestData(DataStructure& dataStructure, usize dimX, usize dimY, usize dimZ, usize blockSize)
{
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    ...
    SIMPLNX_RESULT_REQUIRE_VALID(featureIdsStore.copyFromBuffer(zOffset, nonstd::span<const int32>(featureIdsBuf.data(), sliceSize)));
...
usize CountBadVoxels(const DataStructure& dataStructure, usize dimX, usize dimY, usize dimZ)
{
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    SIMPLNX_RESULT_REQUIRE_VALID(featureIds.copyIntoBuffer(z * sliceSize, nonstd::span<int32>(buf.data(), sliceSize)));
...
TEST_CASE("SimplnxCore::ErodeDilateCoordinationNumberFilter: Bulk Storage Failures", "[SimplnxCore][ErodeDilateCoordinationNumberFilter]")
{
  ...
  for(const auto& [label, fault] : faultCases)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      SIMPLNX_RESULT_REQUIRE_VALID(faultStore->copyFromBuffer(0, nonstd::span<const int32>(inputValues.data(), inputValues.size())));
      ...
      faultArrayRef.setDataStore(faultStore);
